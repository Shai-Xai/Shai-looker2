              ...
              return (
                <div key={g}>
                  <div style={{ padding: '7px 12px', fontSize: 11, fontWeight: 700, letterSpacing: 0.3, textTransform: 'uppercase', color: 'var(--muted)', cursor: 'pointer', background: 'var(--hairline)', display: 'flex', gap: 6, alignItems: 'center', position: 'sticky', top: 0 }}
                    ...
                    <span>{expanded ? '▾' : '▸'}</span><span style={{ flex: 1 }}>{g}</span><span style={{ fontWeight: 400 }}>{matches.length}</span>
                  </div>
                  {expanded && matches.map((d) => (
                    <div key={d.name} style={{ padding: '7px 12px 7px 26px', fontSize: 12.5, cursor: 'pointer', fontWeight: d.name === value ? 700 : 400, color: d.name === value ? 'var(--brand)' : 'var(--text)' }}
                      onClick={() => pick(d.name)}>
                      {d.short || d.label}{tag ? tag(d) : ''}
                    </div>
                  ))}
                </div>
              );
