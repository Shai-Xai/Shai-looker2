...
app.get('/api/admin/ai-overview', auth.requireAdmin, (req, res) => {
  ...
  const clients = db.listEntities().map((ent) => {
    ...
    const tunes = users
      ...
      .filter((t) => t.tune);
    return { id: ent.id, name: ent.name, aiContext: (ent.aiContext || '').trim(), owlGuidance: (db.getSetting(`owl_guidance:${ent.id}`, '') || '').trim(), owlMemory: require('./owlMemory').build(db).read(ent.id).map((m) => m.text), events, digests: jobsByEntity[ent.id] || [], readerTunes: tunes };
  });
