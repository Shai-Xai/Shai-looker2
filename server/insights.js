...
function promptRegistry() {
  return [
    ...
    { key: 'ticketDraft', label: 'Ticket drafting', scope: 'Turns a raw internal bug/feature report into a structured engineering ticket (Admin → Tickets)', text: require('./tickets').TICKET_DRAFT_SYSTEM }, { key: 'fanOwl', label: 'Fan Owl (booking guide)', scope: 'The consumer-facing Owl embedded on promoters\' public event sites — the persona + grounding rules every fan conversation runs under', text: require('./fanOwl').FAN_OWL_SYSTEM }, { key: 'fanIngest', label: 'Fan Owl — website reader', scope: 'Crawls the promoter\'s site and drafts SUGGESTED knowledge entries + page mappings for human review (nothing auto-saves)', text: require('./fanOwl').FAN_INGEST_SYSTEM },
