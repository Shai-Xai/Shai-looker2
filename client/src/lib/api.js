...
export const api = {
  ...
  appAudienceSegment: (entityId, scope, { group, event } = {}) => fetch(scope === 'admin-client' ? `/api/admin/entities/${entityId}/app-audience/segment` : `/api/my/app-audience/${entityId}/segment`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ group, event: event || '' }) }).then(json),
