...
| **Breakdown properties** | which property chips show in "What's driving it" — line order = chip order | `surface`, `cta_label`, `interaction_type`, `$os_name`, `$device_type`, `$device_name` — now the built-in default (OS + device splits ride the same card) |
