...
- **2026-07-23** — **📝 Report a bug: review your ticket before it sends** ✅: when you file
  ...
  **editable**, so what lands on the product board (and what the AI builder works from) is
  exactly what you approved. Your original words and attachments still travel with the
  ticket. If the drafter is unavailable it simply submits the old way — a report is never
  blocked on AI.

- **2026-07-22** — **📣 Report updates by email, at every stage** ✅: when you report a bug,
  improvement or idea, Pulse now **emails you at each stage it moves through** — being
  reviewed, accepted, building started, ready to test, shipped/live, or declined (with the
  reason) — not just at the end. Emails carry your **client branding** (staff reports use the
  platform brand), the report title and a link back to **My reports**, and each stage sends
...
- **2026-07-15** — **Fan Owl: desktop main view ↔ side panel + the bar wears the
  ...
  switches to the classic side panel and back, remembered for the visit. Fixed:
  the persistent bar now honours the site's nav-style setting (inline icons,
  labelled pills above/below the input, ＋ menu, or off) — previously only the
  chat did, so the setting looked dead on bar-mode sites.
