// ─── Help-bot knowledge seed (authored at source) ────────────────────────────
// The starter corpus for the in-app help bot (server/helpBot.js). Authored here
// as part of the change that ships it — version-controlled and reviewed in the
// PR — then applied once on boot. Each article's stable `slug` is recorded in the
// `help_seed_applied` setting, so re-deploys never duplicate an article nor
// resurrect one an admin has since edited or deleted. Admins can add/edit/remove
// articles at runtime (Admin → Product → Help bot) with no deploy — this is only
...
// event. `deepLink` is the in-app screen path the bot can point the user to.
...
const SEED = [
  ...
  },
  {
    ...
    body: 'Pulse has AI woven throughout. Each dashboard tile has an "Explain this" insight, and your home briefing is AI-written. For open-ended questions about your DATA ("how are sales tracking?", "top ticket types?"), ask the Owl — the data analyst — which pulls live, scoped answers. (This help assistant is different: I explain how to USE Pulse, not your sales figures — ask the Owl for numbers.)',
    deepLink: '/ask',
  },
  {
