...
function mount(app, { db, auth }) {
  ...
  sql.exec(`CREATE TABLE IF NOT EXISTS campaign_templates (
    ...
    content_mode TEXT NOT NULL DEFAULT 'template',  -- 'template' | 'html'
  ...
  const clean = (b = {}) => ({
    ...
    contentMode: b.contentMode === 'html' ? 'html' : 'template',
  ...
  const row = (r) => ({ id: r.id, entityId: r.entity_id, name: r.name, subject: r.subject, contentMode: r.content_mode, body: r.body, customHtml: r.custom_html, heroImage: r.hero_image, ctaText: r.cta_text, createdBy: r.created_by, createdAt: r.created_at, updatedAt: r.updated_at });
  ...
  app.post('/api/campaign-templates/:entityId', auth.requireAuth, auth.requirePermission('campaigns.approve'), (req, res) => {
    ...
    sql.prepare('INSERT INTO campaign_templates (id,entity_id,name,subject,content_mode,body,custom_html,hero_image,cta_text,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)')
      .run(id, req.params.entityId, c.name, c.subject, c.contentMode, c.body, c.customHtml, c.heroImage, c.ctaText, req.user.email, ts, ts);
  ...
  app.put('/api/campaign-templates/:entityId/:id', auth.requireAuth, auth.requirePermission('campaigns.approve'), (req, res) => {
    ...
    const c = clean(req.body || {});
    sql.prepare('UPDATE campaign_templates SET name=?,subject=?,content_mode=?,body=?,custom_html=?,hero_image=?,cta_text=?,updated_at=? WHERE id=?')
      .run(c.name, c.subject, c.contentMode, c.body, c.customHtml, c.heroImage, c.ctaText, now(), req.params.id);
