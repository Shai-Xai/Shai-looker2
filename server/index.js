...
const eventopsApi = require('./eventops').mount(app, { db, auth, push, messaging, mailer }); const staffAlertsApi = require('./staffAlerts').mount(app, { db, auth, mailer, push, messaging }) || {}; staffInboundFn = staffAlertsApi.staffInbound; const livepulseApi = require('./livepulse').mount(app, { db, auth, resolveTileValue, resolveTileRows, resolveCustomMetric, resolveEventDate, os, mailer, messaging, eventops: eventopsApi, push, signalFlow: staffAlertsApi.signalFlow }); require('./surveys').mount(app, { db, auth, rateLimit, listEntityEventIds: (eid) => posthogApi.eventIdsForEntity(eid) }); // device/station logistics + 🚨 staff alerts + Live Pulse recurring event-day updates (the Alerts page's "Live updates" tab) + post-event fan surveys (Pulse ⇄ Howler app, docs/specs/SURVEY_CONTRACT.md) → server/eventops.js, server/staffAlerts.js, server/livepulse.js, server/surveys.js
