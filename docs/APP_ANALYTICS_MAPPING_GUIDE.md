...
| **Breakdown properties** | which property chips show in "What's driving it" — line order = chip order | `surface`, `cta_label`, `interaction_type`, `$os_name`, `$device_type`, `$device_model` — now the built-in default. Device ids render as marketing names (`pa3q` → Galaxy S25 Ultra, `iPhone14,2` → iPhone 13 Pro) |
