...
import { goalState, fmtVal, statusChip, bandChip, Chip } from './GoalViz.jsx';
...
//
// Colour choice: rings use a fixed, vivid per-ring palette (so concentric rings stay
// visually distinct even when several goals are at the same %, exactly like Apple) —
// the *meaning* (ahead / behind / hit) rides on the small chip + value in the legend,
// which keeps our semantic state colours. North Star leads (outermost).

// Vivid, Apple-ish identity palette — distinct hues that read on light and dark.
const RING_PALETTE = ['#FA114F', '#A6FF00', '#22D3EE', '#BF5AF2', '#FF9F0A', '#FFD60A', '#0A84FF'];
export const ringColor = (i) => RING_PALETTE[i % RING_PALETTE.length];
...
export default function GoalRingsCard({ goals = [], title, onPick, size = 176, maxRings = 6 }) {
  ...
  const rings = shown.map((g, i) => ({ pct: g.progress?.pct, color: ringColor(i) }));
        ...
        {shown.map((g, i) => {
          ...
          return (
            <button key={g.id} onClick={() => onPick?.(g)} style={legendRow} className="lift">
              <span style={{ width: 9, height: 9, borderRadius: 980, background: ringColor(i), flexShrink: 0, boxShadow: `0 0 6px ${ringColor(i)}88` }} />
              <span style={{ display: 'flex', flexDirection: 'column', minWidth: 0, flex: 1, gap: 1, textAlign: 'left' }}>
                ...
                <span style={{ fontSize: 16, fontWeight: 800, color: ringColor(i), lineHeight: 1.1 }}>
