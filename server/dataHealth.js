...
2. "Flow:" the pipe numbers — minutes since the latest record vs the warn/stale thresholds, total scans, and the busiest period if the timeline shows one.
3. "Devices:" online vs linked, and the scans-per-device spread (call out devices far below the average — they may be weak even if currently online).
4. "Concerns:" a numbered list, worst first. For each: what, the evidence (times, counts, gap lengths), and ONE concrete action (→ send a runner / swap / reboot / check battery or signal). If a device died, say when and for how long. If a device flaps (active-silent-active), say so — that pattern fits battery/signal.
5. End with what you RULED OUT: if other devices ran clean through the same period, say it's device-level, not the pipe or venue network. If MANY devices share the same silent blocks, say the opposite — that pattern fits a venue network/connectivity problem worth raising with the network provider.

Rules: plain text (no markdown headings/tables), ≤ 180 words, every number from the JSON — never invent. Times are UTC ISO in the data; present them as South Africa time (UTC+2), HH:MM. If data is missing (rosterError/timelineError), say what you couldn't see. No greetings, no fluff.`;
