                ...
                {groups.map((c) => (
                  <div key={c.id} style={{ ...card, marginBottom: 0, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: 160 }}>
                      ...
                      <p style={{ margin: '2px 0 0', fontSize: 12, color: 'var(--muted)' }}>by {c.createdBy || 'a fan'} · {c.memberCount} member{c.memberCount === 1 ? '' : 's'} · 👤 {c.chatterCount ?? 0} chatting · 💬 {c.messageCount ?? 0} message{c.messageCount === 1 ? '' : 's'}</p>
                    </div>
    ...
    <div style={{ ...card, marginBottom: 0 }}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 170 }}>
          ...
          {/* Public channels have no member rows (nobody "joins" an open room) —
              show who's actually chatting instead of a misleading "0 members". */}
          <p style={{ margin: '2px 0 0', fontSize: 12, color: 'var(--muted)' }}>{ACCESS_LABEL[c.access]}{c.access === 'tickets' ? ` · ${(c.ticketTypes || []).length ? c.ticketTypes.join(', ') : 'any ticket holder'}` : ''}{c.segmentId ? ` · segment ${c.segmentId}` : ''}{c.access === 'public' ? ` · 👤 ${c.chatterCount ?? 0} chatting` : ` · ${c.memberCount} member${c.memberCount === 1 ? '' : 's'}`} · 💬 {c.messageCount ?? 0} message{c.messageCount === 1 ? '' : 's'}</p>
        </div>
