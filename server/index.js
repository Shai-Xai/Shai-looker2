...
const eventopsApi = require('./eventops').mount(app, { db, auth, push, messaging, mailer }); staffInboundFn = (require('./staffAlerts').mount(app, { db, auth, mailer, push, messaging }) || {}).staffInbound; const livepulseApi = require('./livepulse').mount(app, { db, auth, resolveTileValue, resolveTileRows, resolveCustomMetric, resolveEventDate, os, mailer, messaging, eventops: eventopsApi, push }); // device/station logistics + 🚨 staff alerts + Live Pulse recurring event-day updates (the Alerts page's "Live updates" tab) → server/eventops.js, server/staffAlerts.js, server/livepulse.js
