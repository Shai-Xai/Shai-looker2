...
s.addText('HOWLER PRESENTS', { x: 0.9, y: 1.5, w: 10, h: 0.4, fontSize: 13, bold: true, charSpacing: 3, color: RED, fontFace: F });
...
s.addImage({ path: 'dash-generic.png', x: 1.67, y: 2.5, w: 10.0, h: 2.82 });
s.addText('Live dashboards with the Owl one tap away. Screens and numbers are illustrative.', { x: 1.67, y: 5.42, w: 10.0, h: 0.35, fontSize: 10, italic: true, color: GREY, align: 'center', fontFace: F });
...
const six = [
  ...
  ['🎯', 'Prove the results', 'Set the target that matters, like your ticket-sales North Star, and every screen shows how you’re tracking.'],
...
s.addImage({ path: 'visrow-generic.png', x: 1.77, y: 1.45, w: 9.8, h: 4.5 });
s.addText('Left to right: the morning briefing in the app, the Owl on WhatsApp, and the Pulse connector answering in ChatGPT and Claude.', { x: 1.77, y: 6.0, w: 9.8, h: 0.35, fontSize: 10, italic: true, color: GREY, align: 'center', fontFace: F });
...
s.addText('Your ticketing and cashless data is already flowing through the Howler stack. Pulse is the switch that turns it into insight, action and results.', { x: 0.9, y: 2.7, w: 11.2, h: 1.0, fontSize: 20, color: 'D8D4F0', fontFace: F, lineSpacing: 28 });
s.addText('Let’s show you, live on your own numbers.', { x: 0.9, y: 3.9, w: 11.2, h: 0.6, fontSize: 20, bold: true, color: ORANGE, fontFace: F });
...
p.writeFile({ fileName: 'pulse-deck-lite-generic.pptx' }).then(() => console.log('lite deck written'));
