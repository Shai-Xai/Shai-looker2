...
export const api = {
  ...
  myAppPeople: (entityId, { days, from, to, q, offset, orderBy, limit } = {}) => fetch(`/api/my/app-analytics/${entityId}/people?days=${days || ''}&from=${from || ''}&to=${to || ''}&q=${encodeURIComponent(q || '')}&offset=${offset || 0}&orderBy=${orderBy || ''}&limit=${limit || ''}`).then(json),
  ...
  adminAppBreakdownSeries: ({ key, days, from, to, entityId } = {}) => fetch(`/api/admin/app-analytics/breakdown-series?key=${encodeURIComponent(key)}&days=${days || ''}&from=${from || ''}&to=${to || ''}&entityId=${encodeURIComponent(entityId || '')}`).then(json),
  myAppBreakdownSeries: (entityId, { key, days, from, to } = {}) => fetch(`/api/my/app-analytics/${entityId}/breakdown-series?key=${encodeURIComponent(key)}&days=${days || ''}&from=${from || ''}&to=${to || ''}`).then(json),
