...
require('./supportOwl').mount(app, { db, auth, rateLimit }); // Support Owl P0a: the customer-support knowledge spine, HelpDocs sync + platform-tier curation (docs/specs/SUPPORT_OWL_SPEC.md; kill switch support_owl_enabled) → server/supportOwl.js
...
// 🖼️ Event Media: per-event app assets (header image/video, logo) published from Pulse and overlaid by the Howler app on its Howler GraphQL data — Phase-1 of event setup in Pulse (flag: eventassets) → server/eventAssets.js
require('./eventAssets').mount(app, { db, auth });
