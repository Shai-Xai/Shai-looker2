...
test('commerce scan sweeps a year of event names + mapped property values for order terms', async () => {
  ...
  assert.equal(queries.length, 1 + 7, 'one event sweep + one per unique mapped property (incl. OS/device/model)');
...
test('a legacy stored mapping heals itself on mount — once, keeping custom values', () => {
  ...
  assert.deepEqual(m.breakdownProps, ['surface', 'cta_label', 'interaction_type', '$os_name', '$device_type', '$device_name', '$device_model'], 'CTA_Label swapped for cta_label, surface-first order, OS/device/model appended (v3+v7/v8)');
  ...
  assert.equal(h.settings.posthog_map_healed, '8');
  ...
  assert.equal(h2.settings.posthog_map_healed, '8');
  ...
  const h3 = makeHarness({ presetSettings: { posthog_metric_map: done, posthog_map_healed: '8' } });
...
test('reports carry the configured breakdown keys and live window uniques', async () => {
  const h = makeHarness({ responder: syncResponder });
  await h.api.syncDaily(7);
  assert.deepEqual(h.api.appReport(7).breakdowns, ['surface', 'cta_label', 'interaction_type', '$os_name', '$device_type', '$device_name', '$device_model']);
...
test('history search sweeps event names AND breakdown values over a year, escaped, admin only', async () => {
  ...
  assert.equal(queries.length, 8, 'one event-name sweep + one per configured breakdown property (incl. OS/device/model)');
