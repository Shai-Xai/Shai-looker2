...
- TODAY: the current calendar date. Anchor every "today/yesterday/this month/day N/month-to-date" reference to TODAY, never to the latest date in the data. If the data lags TODAY, say so (e.g. "latest figures are to the 12th") rather than implying that day is now. If a DATA FRESHNESS note marks an event STALE, treat it as concluded (or its feed as stopped): a short wrap-up, never a "live" review of its last data day.
...
- Anchor every time reference — "today", "yesterday", "so far this month", "day N", month-to-date — to TODAY's calendar date, NEVER to the latest date in the data. The pipeline can lag a few days: if the most recent data point is older than TODAY, say so plainly (e.g. "latest figures are to the 12th") instead of calling that day today or yesterday. Don't write "through day N" unless N is TODAY's day-of-month; if the data ends earlier, frame it as "data to the Nth" so it never looks like the month stopped there. When a DATA FRESHNESS note marks an event STALE (its latest data point is days before TODAY), that event is over or its feed has stopped: open by stating the data runs to that date, write it as a short retrospective wrap-up (final totals, what stood out), and NEVER narrate its last data day as "yesterday's trading" or an ongoing run — don't keep re-analysing a finished event as if it were live.
...
You are given:
- TODAY: the date this digest is sent. This — not the data — is the current date; anchor every "today/yesterday/this month/day N" to it. The pipeline can lag a few days: if the latest data point is older than TODAY, say "data to the Nth" rather than calling it today. If a DATA FRESHNESS note marks an event STALE, that event is over or its feed has stopped — give it a one-line wrap-up in the overview, never present its last data day as current trading.
...
function promptRegistry() {
  return [
    ...
    { key: 'designEmail', label: 'Email layout designer', scope: 'The Owl designs a full themed block email (theme + content blocks) from a goal', text: require('./emailDesign').DESIGN_EMAIL_SYSTEM }, ...require('./journeys').promptRegistry(), ...require('./posthog').promptRegistry(), ...require('./opsTriage').promptRegistry(),
