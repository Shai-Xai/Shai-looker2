...
module.exports = function createOwlTools({ query, auth, db, getGoalsApi, getAlertsApi, getCampaignsApi, getUploadsApi, getDriveApi, getMetaAdsApi, resolveTileValue, getExploreFields, getFieldOverrides, draftCampaignCopy, designEmailFn, getSegmentsApi, getEventOpsApi, getDataHealthApi, getSignalFlow, getChottuApi, catalogue = defaultCatalogue }) {
  ...
  // ── createLink (ACT) ──────────────────────────────────────────────────────────
  // DRAFT one ChottuLink deep link (howler.chottu.link/… into the Howler app),
  // tied to the current event. Self-confirm pattern: nothing is created until the
  // user taps "Create link" (committed via POST /api/owl/act/create-chottu-link,
  // which re-checks campaigns.approve — see server/chottuLink.js).
  async function runCreateLink(args = {}, ctx = {}) {
    ...
    if (!chottu) return refuse('unavailable', 'Links aren\'t available right now.');
    const cfg = chottu.configFor(entityId);
    if (!cfg.key || !cfg.domain) return refuse('not_connected', 'ChottuLink isn\'t connected for this client yet — the key + domain live under Settings → Integrations.');
  ...
  // ── applyLinkTemplate (ACT) ───────────────────────────────────────────────────
  // DRAFT the one-click link set for the current event from a saved template
  // (e.g. "Standard event set" = main + ticket wallet + lineup + map + feed +
  // chat). Resolves the full preview server-side; commits via
  // POST /api/owl/act/apply-chottu-template after the user confirms.
